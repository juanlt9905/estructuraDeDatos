...
TNodo *  sucesores(TNodo ** nodo_actual){ //Funcion para crear los nuevos sucesores de un estado
    ...
    while(nelementos > 0){
        ...
        //printf("Revision de sucesores: nodo:(%d, %d), padre: (%d, %d)", )    
...
TNodo * reconstruirCamino(TNodo * nodo_meta, TNodo * cerrados) {
    ...
    while(recorre_por_padres !=NULL){
       ...
       // printf("ANtes de insertar: (%d, %d), padre: (%d, %d)", recorre_por_padres->estado.jarra4, recorre_por_padres->estado.jarra3, recorre_por_padres->padre->estado.jarra4, recorre_por_padres->padre->estado.jarra3);
        AgregarInicio_conPadre(&camino, recorre_por_padres->estado, recorre_por_padres->padre);
        //printf("Estado agregado a CAMINO: (%d, %d) padre (%d, %d)\n", recorre_por_padres->estado.jarra4, recorre_por_padres->estado.jarra3, recorre_por_padres->padre->estado.jarra4, recorre_por_padres->padre->estado.jarra3);
        //moverse al padre
...
TNodo *busquedaPorAnchura(TNodo *nodo_inicial){ //regresa el camino (lista) a la solucion
    ...
    //printf("Estado agregado a abiertos: (%d, %d)\n padre :", abiertos->estado.jarra4, abiertos->estado.jarra3);
    ...
    while(abiertos!=NULL){
        ...
        //if (nodo_actual->padre!=NULL)printf("Estado agregado a cerrados: (%d, %d)\n padre (%d, %d):", nodo_actual->estado.jarra4, nodo_actual->estado.jarra3, nodo_actual->padre->estado.jarra4, nodo_actual->padre->estado.jarra3);
...
int main(){
    ...
    inicializarJarras(nodo_inicial);
    
    //TEst de reconstruir camino.
    /*TNodo *cerrados=NULL;
    TNodo * nodo2 =NULL;
    nodo2=(TNodo*) malloc(sizeof(TNodo));

    nodo2->padre = nodo_inicial;
    nodo2->estado.jarra4=2;
    nodo2->estado.jarra3=0;
    AgregarInicio_conPadre(&cerrados, nodo_inicial->estado, NULL);
    AgregarInicio_conPadre(&cerrados, nodo2->estado, nodo2->padre);

    TNodo *camino= reconstruirCamino(nodo2,cerrados);*/
...
/*TNodo *nodo_inicial=NULL;
    nodo_inicial=(TNodo*) malloc(sizeof(TNodo)); //memoria para el nuevo nodo
    inicializarJarras(nodo_inicial);
    TNodo * lista_sucesores = sucesores(&nodo_inicial);

    printf("Sucesores del nodo inicial: \n");
    VerTodos(lista_sucesores);
    
    TNodo *sucesores2 = sucesores(&lista_sucesores);
    VerTodos(sucesores2);

    TNodo *sucesores3 = sucesores2->sig;
    VerTodos(sucesores(&sucesores3)); */